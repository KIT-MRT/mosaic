...
class SafeVideoGenerator:
    ...
    def _generate_frame_image(self, frame_data):
        ...
        try:
            ...
            plt.suptitle(
                f"nuPlan Trajectory Evaluation - Frame {frame_data['frame']}",
                fontsize=16,
            )
            plt.tight_layout()
    ...
    def _plot_comprehensive_score_comparison(self, ax, frame_data):
        ...
        ax.clear()
        ax.set_title(
            "Comprehensive Score Comparison (All Sub-metrics)",
            fontsize=12,
            fontweight="bold",
        )
        ...
        # 添加分数值
        for i, (bar1, bar2, open_score, closed_score) in enumerate(
            zip(bars1, bars2, open_scores, closed_scores)
        ):
            if open_score > 5:
                ax.text(
                    open_score / 2,
    ...
    def _plot_decision_thinking_chain(self, ax, frame_data):
        ...
        nodes_data = [
            (
                ...
                "red"
                if step1_active
                else ("green" if step1_color == "green" else "orange"),
                step1_active,
            ),
            (
                "step2",
                "STEP 2\nSafety Difference Check",
                step2_text,
                "red" if step2_active else step2_color,
                step2_active,
            ),
            (
                "step3",
                ...
                step3_text,
                "red" if step3_active else step3_color,
                step3_active,
            ),
        ]
        ...
        ax.text(
            result_x,
            result_y - 0.06,
            f"Score: {open_total if winner == 'Open' else closed_total:.1f}",
            transform=ax.transAxes,
            fontsize=10,
            ha="center",
            va="center",
        ...
        if decision_made_at not in ["step1"]:
            ...
            ax.annotate(
                "",
                xy=(
                    node_positions["step2"][0],
                    node_positions["step2"][1] + node_radius,
                ),
                xytext=(
                    node_positions["step1"][0],
                    node_positions["step1"][1] - node_radius,
                ),
        ...
        if decision_made_at not in ["step1", "step2"]:
            ...
            ax.annotate(
                "",
                xy=(
                    node_positions["step3"][0],
                    node_positions["step3"][1] + node_radius,
                ),
                xytext=(
                    node_positions["step2"][0],
                    node_positions["step2"][1] - node_radius,
        ...
        if decision_made_at:
            ...
            ax.annotate(
                "",
                xy=(result_x - node_radius * 1.2, result_y),
                xytext=(decision_node_pos[0] + node_radius, decision_node_pos[1]),
                arrowprops=decision_arrow_props,
                transform=ax.transAxes,
            )

            # 在箭头上添加决策原因
            mid_x = (
                decision_node_pos[0] + node_radius + result_x - node_radius * 1.2
            ) / 2
        ...
        # 添加详细信息
        info_text = (
            f"Score Comparison: Open {open_total:.1f} vs Closed {closed_total:.1f}\n"
        )
        info_text += (
            f"Safety Comparison: Open {open_safety:.1f} vs Closed {closed_safety:.1f}\n"
        )
    ...
    def _plot_score_history(self, ax):
        ...
        if frames:
            ...
            if current_open > current_closed:
                ax.scatter(
                    current_frame, current_open, s=80, c="orange", marker="*", zorder=5
                )
            else:
                ax.scatter(
                    current_frame, current_closed, s=80, c="green", marker="*", zorder=5
                )
    ...
    def _plot_environment_and_trajectories(self, ax, frame_data):
        ...
        if "ego_state" not in frame_data:
            ax.text(
                0.5, 0.5, "Environment data not available", ha="center", va="center"
            )
        ...
        # 绘制ego车辆中心点和方向箭头
        ax.scatter(
            ego_center_x, ego_center_y, c="darkblue", s=30, marker="o", zorder=10
        )
        ...
        for obj in surrounding_objects:
            ...
            if "pedestrian" in obj.object_type.lower():
                ...
                ax.scatter(
                    ...
                    marker="o",
                    edgecolors="black",
                    linewidth=2 if is_closest else 1,
                    alpha=0.8,
                    zorder=5,
                )
            else:  # vehicle
                ...
                obj_rect = patches.Rectangle(
                    ...
                    facecolor=color,
                    alpha=0.7 if is_closest else 0.5,
                    edgecolor="black",
                    linewidth=2 if is_closest else 1,
                )
        ...
        if trajectory1:
            ...
            if trajectory1:
                ...
                dist_to_ego = math.sqrt(
                    (first_point[0] - ego_center_x) ** 2
                    + (first_point[1] - ego_center_y) ** 2
                )
                ...
                if dist_to_ego > 0.5:  # 如果距离超过0.5米，说明轨迹起点不在ego中心
                    ...
                    # 绘制从ego中心到轨迹起点的连接线
                    ax.plot(
                        [ego_center_x, first_point[0]],
                        [ego_center_y, first_point[1]],
                        color="orange",
                        linestyle=":",
                        linewidth=2,
                        alpha=0.7,
        ...
        if closest_obj:
            ax.plot(
                [ego_center_x, closest_obj.x],
                [ego_center_y, closest_obj.y],
                "r--",
                linewidth=2,
                alpha=0.7,
                label=f"Closest: {min_dist:.1f}m",
                zorder=7,
            )
        ...
        if trajectory1 and trajectory1:
            dist1 = math.sqrt(
                (trajectory1[0][0] - ego_center_x) ** 2
                + (trajectory1[0][1] - ego_center_y) ** 2
            )
            info_text += f"Open traj start dist: {dist1:.2f}m\n"

        if trajectory2 and trajectory2:
            dist2 = math.sqrt(
                (trajectory2[0][0] - ego_center_x) ** 2
                + (trajectory2[0][1] - ego_center_y) ** 2
            )
    ...
    def _draw_road_network(self, ax, ego_state, plot_range):
        ...
        # 绘制垂直和水平道路线
        for x in range(
            int(min_x // road_spacing) * road_spacing,
        ...
        for y in range(
            int(min_y // road_spacing) * road_spacing,
        ...
        for offset in [-lane_width / 2, lane_width / 2]:
            ax.plot(
                [min_x, max_x],
                [ego_road_y + offset, ego_road_y + offset],
                "white",
                alpha=0.5,
                linewidth=1,
                linestyle="--",
            ...
            ax.plot(
                [ego_road_x + offset, ego_road_x + offset],
                [min_y, max_y],
                "white",
                alpha=0.5,
                linewidth=1,
    ...
    def generate_video_from_images(
        self, fps=8, output_name="trajectory_evaluation.mp4"
    ):
        ...
        try:
            cmd = [
                ...
                str(fps),
                "-i",
                str(self.output_dir / "frame_%06d.png"),
                "-c:v",
                "libx264",
                "-pix_fmt",
                "yuv420p",
...
class EgoAgent(AbstractPlanner):
    ...
    def compute_planner_trajectory(
        ...
        self.video_generator.save_frame_data(
            ego_state,
            surrounding_objects,
            traj1_points,
            traj2_points,
...
print(
    "✅ 成功生成 aggregator_metric.parquet！你可以刷新 nuBoard 看 Evaluation Score 了。"
)
# 修复不连续编号的图片文件并生成视频
import shutil
import subprocess
...
def fix_video_generation_for_sparse_frames():
    ...
    cmd1 = [
        ...
        "-vf",
        "scale=trunc(iw/2)*2:trunc(ih/2)*2",  # 确保偶数尺寸
        "-c:v",
        "libx264",
        "-pix_fmt",
        "yuv420p",
    ...
    cmd2 = [
        ...
        "glob",
        "-i",
        str(image_dir / "frame_*.png"),
        "-vf",
        "scale=1588:1180",  # 固定尺寸
        "-c:v",
        "libx264",
        "-pix_fmt",
        "yuv420p",
...
def main_fixed_video_generation():
    ...
    if success:
        ...
        # 检查第一张图片的详细信息
        image_files = list(
            Path("/workspace/nuplan-devkit/visualizations").glob("frame_*.png")
        )
        if image_files:
